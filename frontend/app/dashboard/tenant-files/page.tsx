...
import { 
  ...
  Globe, Plus, MessageSquare, Loader2, Sparkles, Bot, Send, History, X, FileText 
} from "lucide-react";
...
export default function TenantFilesPage() {
  ...
  const handleSaveAndAnalyze = async () => {
    ...
    setIsAnalyzing(true);
    const analysisToast = toast.loading("Analyzing contract with DeepAgents...");
    ...
    try {
      ...
      toast.success("Analysis complete. Risk profile updated!", { id: analysisToast });
    } catch (error) {
  ...
  const showMockGenerated = !contractId && !fetching && searchParams.get('generated') === 'true';
  ...
  if (fetching) {
    return (
      <div className="flex w-full h-[calc(100vh-70px)] items-center justify-center bg-white flex-col gap-4">
        ...
        <p className="text-slate-500 font-bold tracking-widest text-[12px] uppercase">Retrieving Analysis...</p>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between px-10 py-5 border-b border-slate-100 bg-white shrink-0">
          <div className="flex items-center gap-4">
            <h2 className="text-[11px] font-bold tracking-widest text-[#0B1437] opacity-60 uppercase">
              DOCUMENT / {contract?.filename || "No Document Selected"}
            </h2>
            <span className={`px-2.5 py-1.5 rounded-md text-[9px] font-bold tracking-widest uppercase transition-colors ${
              ...
              {hasUnsavedChanges ? "UNSAVED CHANGES" : contract?.status || "DRAFT"}
            </span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-16 py-12 relative bg-white custom-scrollbar">
          {!contractId ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-10">
              ...
              <h3 className="text-[20px] font-bold text-slate-900 mb-2">No Document Loaded</h3>
              <p className="text-slate-500 text-[14px] max-w-sm">Please select a contract from your repository to view and analyze its content.</p>
            </div>
          ) : (
            <div className="max-w-[750px] mx-auto space-y-12">
              ...
              >
                <h1 className="text-[32px] font-bold text-[#0B1437] mb-6 tracking-tight">
                  {contract?.filename?.split('.')[0]?.replaceAll('_', ' ') || "MASTER SERVICE AGREEMENT"}
                </h1>
                <p className="text-[14px] text-slate-600 leading-[1.8] font-medium">
                  {analysis?.clauses?.[0]?.raw_text || "This agreement is currently being indexed by our systems. Please wait for the initial processing to complete before full editing."}
                </p>
              </div>

              {analysis?.clauses?.slice(1).map((clause: any, idx: number) => (
                <div key={idx} className={`relative p-4 -mx-4 rounded-2xl transition-colors ${
                  clause.severity === 'critical' || clause.severity === 'high' ? 'bg-red-50/30' : 'focus:bg-slate-50/50'
                }`}>
                  <p className="text-[10px] font-bold tracking-[0.15em] text-[#3B5FE5] uppercase mb-4">
                    SECTION {clause.clause_number} / {clause.clause_type?.replaceAll('_', ' ')}
                  </p>
                  <p 
                    contentEditable 
                    suppressContentEditableWarning 
                    onInput={handleContentChange}
                    className="text-[15px] text-slate-700 leading-[2] font-medium outline-none"
                  >
                    {clause.raw_text}
                  </p>
                  {clause.severity !== 'info' && (
                    <span className={`mt-4 inline-flex items-center gap-1.5 px-3 py-1.5 ${getSeverityBg(clause.severity)} ${getSeverityColor(clause.severity)} text-[9px] font-bold tracking-widest uppercase rounded`}>
                      AUTO-ANALYSIS: {clause.severity}
                    </span>
                  )}
                </div>
              ))}
      ...
      {isChatOpen ? (
        ...
        <div className="flex-1 min-w-[380px] max-w-[450px] bg-[#F4F7FB] overflow-y-auto h-full px-8 py-10 space-y-10 custom-scrollbar shadow-[-10px_0_30px_rgba(0,0,0,0.02)] relative animate-in slide-in-from-right duration-300">
           <div className="flex items-center justify-between pb-2 border-b border-slate-200/60">
              ...
              <button onClick={() => setIsChatOpen(false)} className="w-10 h-10 rounded-full bg-slate-200/50 hover:bg-slate-200 text-slate-500 flex items-center justify-center transition-colors">
                <X className="w-4 h-4" />
              </button>
           </div>
           ...
           <div className="space-y-4">
              ...
              <div className="bg-white rounded-[20px] p-7 shadow-sm border border-slate-100">
                 <p className="text-[13px] text-slate-700 leading-relaxed font-medium mb-6">
                    {analysis?.analysis_summary || "Performing strategic evaluation of document structure..."}
                 </p>

                 <div className="space-y-3">
                    {analysis?.recommended_actions?.slice(0, 3).map((rec: string, i: number) => (
                      <div key={i} className="bg-blue-50/80 rounded-xl p-4 border border-blue-100/50">
                        <div className="flex items-center gap-2 mb-2">
                            <Info className="w-4 h-4 text-[#3B5FE5]" strokeWidth={2.5}/>
                            <span className="text-[10px] font-bold tracking-widest text-[#3B5FE5] uppercase">Recommendation {i+1}</span>
                        </div>
                 ...
                 {analysis?.detailed_suggestions && (
                   <div className="mt-8 pt-8 border-t border-slate-100 space-y-6">
                      <div className="flex items-center justify-between">
                         <h4 className="text-[11px] font-bold tracking-[0.12em] uppercase text-slate-400">Strategic Enhancements</h4>
                         <span className="text-[10px] font-bold text-blue-600">{analysis.detailed_suggestions.improvement_potential}% Potential</span>
                      </div>

                      {[
                        { tier: 'tier_1_critical', label: 'Tier 1 / Critical', color: 'red' },
                        { tier: 'tier_2_important', label: 'Tier 2 / Important', color: 'orange' },
                        { tier: 'tier_3_recommended', label: 'Tier 3 / Recommended', color: 'blue' }
                      ].map((t) => {
                        const items = analysis.detailed_suggestions[t.tier as keyof typeof analysis.detailed_suggestions];
                        if (!Array.isArray(items) || items.length === 0) return null;
                        
                        return (
                          <div key={t.tier} className="space-y-3">
                            <span className={`text-[9px] font-black uppercase tracking-widest ${
                              t.color === 'red' ? 'text-red-500' : t.color === 'orange' ? 'text-orange-500' : 'text-blue-500'
                            }`}>
                              {t.label}
                            </span>
                            {items.map((item: any, i: number) => (
                              <div key={i} className="bg-white rounded-xl p-4 border border-slate-100 shadow-sm hover:border-blue-200 transition-colors">
                                <h5 className="text-[12px] font-bold text-slate-900 mb-1">{item.title}</h5>
                                <p className="text-[11px] text-slate-500 leading-normal">{item.purpose}</p>
                              </div>
                            ))}
                          </div>
                        );
        ...
        <div className="flex-1 min-w-[380px] max-w-[450px] bg-[#F4F7FB] overflow-y-auto h-full px-8 py-10 space-y-12 custom-scrollbar shadow-[-10px_0_30px_rgba(0,0,0,0.02)] animate-in slide-in-from-right duration-300">
              ...
              <div className="relative z-10 flex items-start justify-between mb-8">
               ...
               <div className={`relative z-10 transition-opacity duration-300 ${isAnalyzing ? 'opacity-30 blur-[2px]' : hasUnsavedChanges ? 'opacity-60' : 'opacity-100'}`}>
                <div className="flex items-baseline gap-2 mb-8">
                   <span className="text-[64px] font-bold tracking-tighter leading-none">{analysis?.overall_risk_score || 0}</span>
                   <span className="text-[15px] font-medium text-white/50 mb-2">/ 100</span>
                </div>
                <div className="w-full h-1.5 bg-white/10 rounded-full mb-8 overflow-hidden">
                   <div className={`h-full rounded-full transition-all duration-[2000ms] ease-in-out ${
                     (analysis?.overall_risk_score || 0) > 70 ? 'bg-red-400' : (analysis?.overall_risk_score || 0) > 40 ? 'bg-amber-400' : 'bg-blue-400'
                   }`} style={{ width: `${analysis?.overall_risk_score || 0}%` }}></div>
                </div>
                
                {/* Visual Issue Grid */}
                <div className="grid grid-cols-2 gap-3 mb-8">
                  <div className="bg-white/5 rounded-xl p-3 border border-white/5">
                    <span className="block text-[10px] font-bold text-white/40 uppercase mb-1">Critical / High</span>
                    <span className="text-[18px] font-bold text-red-300">{(analysis?.critical_issues || 0) + (analysis?.high_issues || 0)}</span>
                  </div>
                  <div className="bg-white/5 rounded-xl p-3 border border-white/5">
                    <span className="block text-[10px] font-bold text-white/40 uppercase mb-1">Medium / Low</span>
                    <span className="text-[18px] font-bold text-blue-200">{(analysis?.medium_issues || 0) + (analysis?.low_issues || 0)}</span>
                  </div>
                </div>

                <p className="text-[12px] text-white/70 leading-[1.6] font-medium w-[90%]">
                   {analysis?.overall_risk_score > 60 ? "DeepAgents flagged this as high risk. Multiple unfavorable clauses identified requiring negotiation." : "Risk profile appears manageable. Primarily standard market terms detected."}
                </p>
              </div>
           </div>
           ...
           <div className="space-y-4">
              <div className="flex items-center justify-between mb-4">
                 ...
                 <span className="px-3 py-1.5 bg-[#Edf2FF] text-[#3B5FE5] rounded-md text-[9px] font-bold tracking-widest uppercase">{analysis?.clauses?.length || 0} Detected</span>
              </div>
                  ...
                  <div key={i} className="group/clause flex flex-col space-y-4">
                    ...
                    >
                      <div className="flex gap-4">
                        <div className={`w-8 h-8 rounded-full ${clause.severity === 'critical' || clause.severity === 'high' ? 'bg-red-100/50 text-red-500' : 'bg-green-100/50 text-green-500'} flex items-center justify-center shrink-0 mt-0.5`}>
                        ...
                        <div className="flex-1">
                            <div className="flex items-center justify-between mb-2.5 gap-2">
                              <h4 className="font-bold text-[14px] text-slate-900 leading-none capitalize truncate">{clause.clause_type?.replaceAll('_', ' ')}</h4>
                              <span className={`px-2 py-1 ${getSeverityBg(clause.severity)} ${getSeverityColor(clause.severity)} rounded text-[9px] font-bold tracking-widest uppercase shrink-0`}>
                                {clause.severity}
                              </span>
                            </div>
                            ...
                            {isExpanded && (
                              <div className="mt-6 pt-5 border-t border-slate-50 space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
                                <div className="space-y-2">
                                  <div className="flex items-center justify-between">
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Deep Reasoning</span>
                                    <span className="text-[10px] font-bold text-blue-500">{clause.confidence_score}% Confidence</span>
                                  </div>
                                  <div className="bg-slate-50 rounded-xl p-4">
                                     <p className="text-[12px] text-slate-700 leading-relaxed font-medium whitespace-pre-line">
                                       {clause.legal_reasoning || "No automated reasoning available for this section."}
                                     </p>
                                  </div>
                                </div>
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="flex items-center gap-2 px-3 py-2 bg-blue-50 rounded-lg border border-blue-100/50">
                                     ...
                                     <span className="text-[11px] font-bold text-blue-800">{clause.applicable_statute || "Indian Contract Act 1872"} / {clause.statute_section}</span>
                                  </div>
                                </div>
                              </div>
                            )}
