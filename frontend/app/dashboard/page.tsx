...
import { UploadCloud, FileText, FileSearch, XCircle, RefreshCcw, ExternalLink, Plus, Loader2 } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || "Failed to upload contract", { id: uploadToast });
    } finally {
      setUploading(false);
      ...
      <div className="flex flex-col xl:flex-row gap-8 mb-16 relative">
        <div className="flex-1 bg-[#F9FAFC] border-[2px] border-dashed border-[#E2E8F0] rounded-[24px] p-12 lg:p-20 flex flex-col items-center justify-center text-center">
          <div className="w-16 h-16 bg-[#E0E7FF] rounded-2xl flex items-center justify-center mb-6">
            {uploading ? (
                <Loader2 className="w-8 h-8 text-[#0B1437] animate-spin" strokeWidth={2.5} />
            ) : (
        ...
        <div className="bg-white rounded-[20px] shadow-sm border border-slate-100 overflow-hidden">
          {fetching ? (
            <div className="p-20 flex flex-col items-center justify-center gap-4">
              ...
              <p className="text-slate-500 font-medium">Loading repository...</p>
            </div>
          ) : contracts.length === 0 ? (
            <div className="p-20 text-center">
              <FileText className="w-12 h-12 text-slate-200 mx-auto mb-4" />
              <p className="text-slate-500 font-medium">No contracts found. Upload your first document above.</p>
            </div>
          ) : (
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-50">
                {contracts.map((doc) => (
                  <tr key={doc.id} className="hover:bg-[#F9FAFC] transition-colors group">
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center`}>
                          <FileText className={`w-4 h-4 text-[#3B5FE5]`} strokeWidth={2.5}/>
                        </div>
                        <span 
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-2">
                         <div className={`w-1.5 h-1.5 rounded-full ${
                           ...
                           doc.status === 'ANALYSIS_FAILED' ? 'bg-red-500' : 
                           doc.status === 'ANALYZING' ? 'bg-blue-400 animate-pulse' : 'bg-slate-300'
                         }`}></div>
                         <span className={`text-[10px] font-bold tracking-wider uppercase ${
                           doc.status === 'ANALYZED' ? 'text-green-600' :
                           doc.status === 'ANALYSIS_FAILED' ? 'text-red-600' : 'text-slate-500'
                         }`}>
                           {doc.status}
                         </span>
                      </div>
                    ...
                    <td className="px-8 py-5 text-right">
                      <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity float-right">
                        ...
                        >
                          View Analysis
                          <ExternalLink className="w-[18px] h-[18px]" strokeWidth={2.5} />
                        </button>
                      </div>
                    </td>
                  </tr>
